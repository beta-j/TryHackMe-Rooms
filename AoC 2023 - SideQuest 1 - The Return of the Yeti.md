...
We're really close to completing this challenge now.  PyRDP provides us with more information than we could ever ask for - video playback, keystroke logs and clipboard monitoring!  Just by looking through the video generated by PyRDP, we see that our attacker opened Google Chrome and accessed Gmail.  They logged in as `mcskidyelf@gmail.com` with the password `j!*********` and opened an email with the subject line **RE: Suspicious activity** recieved from **Cyber Police**.  In this email we see that the case number assigned is `31337-0`, which they copy to clipboard.  They then reply to the email saying that they will be sending a copy of a *"weird file"* through a *"more secure channel"*.  They also copy this email to the clipboard - so it is most likely that we could have extracted this information directly from within Wireshark if we were more patient (and wanted to miss out on the cool video output we generated).
...
Next the attacker closes the Chrome browser, opens Powershell and navigates to the `DESKTOP` folder. They list the contents of the `DESKTOP` folder and copies the filename for `yetikey1.txt`.  Then they appear to paste in a Powershell command they copied from an other terminal; ``Set-Clipboard-value(Get-Content .\Desktop\secret.txt.txt)`` and edit this to read ``Set-Clipboard-value(Get-Content .\Desktop\yetikey1.txt)`` then run the command three times.  This updates the clipboard with the contents of `yetikey`.txt` - which we can conventiently see from the clipboard monitoring in PyRDP player.
